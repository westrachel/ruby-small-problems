...
#     Instead of the deck being an array w/ sub-hashes for each unique suit + card combo,
#      and each of these sub-hashes containing an additional sub-hash specifying the card
#      and the card value, could just have an array w/ a sub-hash whose key = suit
#      and whose value = card type
#        <=> This removes the extra key/value pair that indicates a "ten" card = 10
#     note: could also change deck to an array of nested arrays containing unique suit card combos
#        <=> an array containing arrays may be even simpler to work w/, but I'm keeping the usage
#          of sub-hashes as practice for working w/ hashes
# ii. In tandem w/ i., don't overwrite sampled ace cards to adjust their value to be either 1 or 11.
#     Instead, alter the card summation logic based on knowledge that a hand w/ 2 or more aces will
#     assign at most one ace's value to 11 and all other aces to a value of 1.
# iii. Can sample a card from the initialized deck and remove it from the initialized deck at the
#       same time. This is more efficient than how I previously sampled a random card and assigned
#       the random card to a local variable, so that I could first push this local random card variable
#       to either the player's hand or the dealer's hand and then iterate back through the initialized
#       deck to remove the local random card variable.
...
def initial_deal_total(hand)
  ...
  hand[0, 2].each do |card_hash|
    if card_hash.values[0] == "Ace"
      sum += 11
    elsif card_hash.values[0].to_i == 0
      sum += 10
    else
      sum += card_hash.values[0].to_i
    end
  end
...
def total(hand)
  ...
  hand[2, last_idx].each do |card_hash|
    if card_hash.values[0] == "Ace" && sum <= 10
      sum += 11
...
# The following method will not declare the correct winner if someone hasn't busted
...
loop do # start of outer full game loop
  ...
  # refactored to no longer use created deal_card! method, but
  # rather use pop method to sample and remove a card from the
  # initialized deck at the same time
  # call shuffle method before call pop method, so that both
  # player and dealer will get 2 random cards not the cards at
  # the very end of the deck
  2.times do
    plyr_hand.push(deck.shuffle.pop)
    dealer_hand.push(deck.shuffle.pop)
  end
  ...
  loop do
    ...
    plyr_hand.push(deck.shuffle.pop)
    p plyr_hand
    last_cd_idx = plyr_hand.size - 1
    prompt "You received the:"
    list "#{card_val(plyr_hand, last_cd_idx)} of #{card_suit(plyr_hand, last_cd_idx)}"
