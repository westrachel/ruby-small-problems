...
until valid_to_two_decimals?(duration_in_yrs)
  puts "That doesn't seem right. Please enter the total number of years of your loan."
  duration_in_yrs = gets.chomp
end

duration_in_months = duration_in_yrs.to_i * 12 * -1
monthly_interest_rate = annual_apr.to_f / 12 / 100 
monthly_payment = (loan_amount.to_f * (monthly_interest_rate / (1 - (1 + monthly_interest_rate)**(duration_in_months)))).round(2)
